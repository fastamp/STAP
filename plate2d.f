...
!     Set up parameters
      ...
      eta1p = 1.0d0 + xi(2)
      eta1m = 1.0d0 - xi(2)
      xi2   = xi1p  * xi1m            !1-\xi^2
      eta2  = eta1p * eta1m           !1-\eta^2
...
!     Construct jacobian-transpose and its inverse
      ...
      do i = 1,2
        do j = 1,2
          jac(i,j) = 0.0d0
          do k = 1,nel
            jac(i,j) = jac(i,j) + xl(j,k)*shp(i,k)          !标准的jacobian矩阵at(\xi,\eta)->OCZ book
          end do
        end do
      end do

      xsj = jac(1,1)*jac(2,2)-jac(1,2)*jac(2,1)             !jacobian行列式at(\xi,\eta)
...
!     Contribution to transverse displacement
      ...
      do i = 1,4
        b1(i)   = jinv(1,1)*shp(1,i)  + jinv(1,2)*shp(2,i)  !N,x
        b2(i)   = jinv(2,1)*shp(1,i)  + jinv(2,2)*shp(2,i)  !N,y
        f1(i)   = jinv(1,1)*shpm(1,i) + jinv(1,2)*shpm(2,i) !N_wt,x
        f1c(i)  = f1(i)*co(i)                               
        f1s(i)  = f1(i)*si(i)
        f2(i)   = jinv(2,1)*shpm(1,i) + jinv(2,2)*shpm(2,i) !N_wt,y
        f2c(i)  = f2(i)*co(i)
        f2s(i)  = f2(i)*si(i)
      end do
...
!     Parameters for bubble functions
      ...
      aa        = jac0(1,1)*dj2 - jac0(1,2)*dj1     !这里按公式应该是jac而不是jac0
